        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10 mb-12">
          ...
          <div>
            ...
            <ul className="space-y-3">
              {[
                { label: "Fertilizers & NPKs", category: "Water Soluble Fertilizers" },
                { label: "Crop Protection", category: "Herbicide/Fungicide/Insecticide" },
                { label: "Biostimulants", category: "Biostimulant" },
                { label: "Micronutrients", category: "Micronutrient" }
              ].map((item) => (
                <li key={item.label}>
                  <Link href={`/products?category=${encodeURIComponent(item.category)}`} className="text-[13px] text-gray-500 hover:text-gray-900 transition-colors">
                    {item.label}
                  </Link>
                </li>
              ))}
